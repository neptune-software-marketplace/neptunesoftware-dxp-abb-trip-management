    ...
    for (i = 0; i < hotelModel.length; i++) {
       ...
       // console.log("HOTEL",detectedCurrency)
        if(detectedCurrency === "EUR") {
            ...
            var selectedExchangeRate = hotelModel[i].price * 1.17;
            //console.log("CONVERTED RATE: " + selectedExchangeRate);
            totalTripCostings = totalTripCostings + selectedExchangeRate;
            //console.log("TOTAL: " + totalTripCostings);
        }
    }
...
function clearForm(){
    ...
    let getValcomponents = [oInputFirstName, oInputLastName, oInputTelephone, oInputEmail, oDatePickerDOB,
     oInputAddress, oInputAddressCity, oInputAddressStateProvinceRegion, oInputAddressPostCode, oInputPassportNumber,
     oInputPassportNationality, oDatePickerPassportExpiryDate, oInputDrivingLicenseNumber, oDatePickerDLExpiryDate];
    
    //Arrow function
    getValcomponents.forEach(component => component.setValue(""));

    oSelectTitle.setValue("");
    oSelectCountry.setValue("");
    oSelectPassportIssueCountry.setValue("");
